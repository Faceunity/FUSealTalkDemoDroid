...
public class MediaCodecWrapper {
    ...
    public static MediaCodecWrapper fromAudioFormat(final MediaFormat trackFormat)
        ...
        // Check to see if this is actually a audio mime type. If it is, then create
        // a codec that can decode this mime type.
    ...
    public boolean writeSample(
        ...
        if (size > 0 && !mAvailableInputBuffers.isEmpty()) {
            ...
            // indicates the position (startPlay time) for the current sample.
            if (crypto == null) {
    ...
    public byte[] popSample() {
        ...
        if (!mAvailableOutputBuffers.isEmpty()) {
            ...
            byte[] sampleData = new byte[bufferInfo.size];
            byteBuffer.get(sampleData);
