...
/**
 * 通用列表 Adapter 类。 支持的数据类型为{@link ListItemModel}. Adapter 可更具View 的资源 id 通过 {@link
 * ViewHolderFactory#createViewHolder(int, View)} 方法去获取对应的ViewHolder 进行使用。
 *
 * <p>所以要特别注意的是。如果想单独使用此 Adapter 的时候。 需要记得提前在 ViewHolderFactory 中添加展示所需的布局 viewResId 和其对应的
 * ViewHolder 的映射关系。否则会找不到ViewHolder 的构造， 从而出现崩溃。
 *
 * <p>如果是和 {@link CommonListBaseViewModel} 类配合使用的话， 则使用其内部类{@link
 * CommonListBaseViewModel.ModelBuilder} 类进行构造的话， 就会自动添加 View 资源和ViewHolder 的映射关系，则无需手动添加。
 *
...
public class CommonListAdapter
        extends ListWithSideBarBaseAdapter<ListItemModel, BaseItemViewHolder> {
    ...
    public BaseItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        final BaseItemViewHolder viewHolder =
                ViewHolderFactory.getInstance().createViewHolder(viewType, view);
        return viewHolder;
    }
    ...
    public void onBindViewHolder(@NonNull BaseItemViewHolder holder, int position) {
        ...
        holder.setOnLongClickItemListener(
        ...
        if (holder instanceof CommonFriendItemViewHolder) {
            if (data.size() - 1 != position) {
                ...
                if (itemModel.getItemView().getItemResId()
                        != (listItemModel.getItemView().getItemResId())) {
    ...
    protected void updateSelectedStatus(
            @NonNull BaseItemViewHolder holder, ListItemModel checkableContactModel, int type) {
    ...
    public int getPositionForSection(int sectionIndex) {
        for (int i = 0; i < getItemCount(); i++) {
            ...
            if (!TextUtils.isEmpty(displayName)
                    && type.getValue() == ListItemModel.ItemView.Type.TEXT.getValue()) {
