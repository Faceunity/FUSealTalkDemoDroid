...
public class FURenderer implements OnFaceUnityControlListener {
    ...
    public void onDrawFrame(final byte[] yBuffer, final byte[] uBuffer, final byte[] vBuffer, final int yStride, final int uStride, final int vStride, final int w, final int h) {
        if (handler == null) {
            return;
        }
            ...
            try {
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        synchronized (lock) {
                            prepareDrawFrame();
                            faceunity.fuRenderToYUVImage(yBuffer, uBuffer, vBuffer, yStride, uStride, vStride, w, h, mFrameId++, mItemsArray);
