...
public class FaceBeautyShapeControlView extends BaseControlView {
    ...
    public void bindDataFactory(AbstractFaceBeautyDataFactory dataFactory) {
        ...
        if (switchCompat != null) {
            switchCompat.setChecked(dataFactory.isFaceBeautyEnabled());
        }
    }
    ...
    private boolean checkFaceShapeChanged() {
        if (mShapeBeauty.size() > mShapeIndex && mShapeBeauty.size() > 0) {
            double value;
            double defaultV;
            if (mShapeIndex >= 0) {
                FaceBeautyBean bean = mShapeBeauty.get(mShapeIndex);
    ...
    private void recoverData(ArrayList<FaceBeautyBean> beautyBeans, int currentIndex) {
        ...
        if (clampIndex(beautyBeans, currentIndex)) {
    ...
    private boolean clampIndex(ArrayList<FaceBeautyBean> beautyBeans, int index) {
        if (beautyBeans.size() == 0) return false;
        if (index < 0) return false;
        return index < beautyBeans.size();
    }
}
